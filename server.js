...
app.use(session({
    ...
    resave: false,require('dotenv').config();
const express = require('express');
const axios = require('axios');
const cors = require('cors');
const path = require('path');
...
app.post('/api/chat', isAuth, async (req, res) => {
    ...
    try {
        ...
        if (!candidates || candidates.length === 0 || !candidates[0].content) {
            ...
            console.log(JSON.stringify({ event: "sophia_interaction", timestamp: new Date().toISOString(), user: userEmail, displayName, action: "blocked", ticketTipo: null, ticketId: null, app: null, resetAction: null, result: reason, responseTimeMs: Date.now() - startTime, chatId, messagePreview: message.substring(0, 80) }));
        ...
        if (part.functionCall) {
            ...
            // 📊 Telemetría: interacción con function call
            console.log(JSON.stringify({
                event: "sophia_interaction",
                timestamp: new Date().toISOString(),
                user: userEmail,
                displayName,
                action: name,
                ticketTipo: args?.tipo || null,
                ticketId: args?.ticket_id || null,
                app: args?.sysapp || null,
                resetAction: args?.action || null,
                result: resultData?.Status || resultData?.Estatus || (resultData?.Error ? "error" : "ok"),
        ...
        // 📊 Telemetría: respuesta de texto sin herramienta
        console.log(JSON.stringify({
            event: "sophia_interaction",
            timestamp: new Date().toISOString(),
            user: userEmail,
        ...
        console.log(JSON.stringify({
            ...
            app: null,
            resetAction: null,
            result: e.code || e.response?.status?.toString() || "exception",
            responseTimeMs: Date.now() - startTime,
            chatId,
            messagePreview: req.body?.message?.substring(0, 80) || ""
        }));
    }
});
